...
import services.StartTheftScreenIfNot;
...
public class TheftScreenActive extends Activity implements OnClickListener {
	Button bOk, bDel;
	Button b[] = new Button[10];
	EditText et[] = new EditText[4];
	static int myVal = 0;
	SharedPreferences pref;
	boolean Antitheft_lock_protection = true;
	TextView tv;
	...
	protected void onStop() {
		...
		Toast.makeText(getApplicationContext(), "STOP", Toast.LENGTH_SHORT)
				.show();
		// if (Antitheft_lock_protection
		// && pref.getString("Antitheft_login_successfully_done", null) != null)
		// startService(new Intent(getApplicationContext(),
		// StartTheftScreenIfNot.class));
		super.onStop();
	}

	private void fraud() {
		// TODO Auto-generated method stub
		Intent i = new Intent(getApplicationContext(), FraudUserDetect.class);
		startService(i);
	}

	private void comformIt() {
		// TODO Auto-generated method stub
		Antitheft_lock_protection = false;
		StartTheftScreenIfNot.goAhead = false;
		DetectSIMChange.goAhead = false;
		// stopService(new Intent(getApplicationContext(),
		// StartTheftScreenIfNot.class));
		// onDestroy();
		// System.exit(0);
		stopService(new Intent(getApplicationContext(), DetectSIMChange.class));
		finish();
